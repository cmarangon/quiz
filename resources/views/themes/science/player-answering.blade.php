@php
    $letters = ['a', 'b', 'c', 'd'];
@endphp
<div class="qz-theme qz-theme--science qz-player" wire:key="player-answering-science-{{ $currentQuestion['question_id'] ?? '' }}">
    ...
    <div class="space-y-5">
        @if(! empty($currentQuestion['category_name']))
            <div class="flex justify-center">
                <span class="qz-badge">⚗️ {{ $currentQuestion['category_name'] }}</span>
            </div>
        @endif
        ...
        <p class="qz-hint">{{ __('Tap your answer before the reaction completes!') }}</p>
    </div>
</div>
