...
@endphp
<div class="qz-theme qz-theme--{{ $theme }} qz-player" wire:key="player-answering-{{ $theme }}-{{ $currentQuestion['question_id'] ?? '' }}" x-data="choiceAnswer()">
    @include('themes.'.$theme.'._deco')

    <div class="space-y-5">
        @if(! empty($currentQuestion['category_name']))
            <div class="flex justify-center">
                <span class="qz-badge">{{ $themeConfig['badge'] }} {{ $currentQuestion['category_name'] }}</span>
            </div>
        @endif
