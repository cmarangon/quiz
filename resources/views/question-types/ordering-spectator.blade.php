...
@else
    ...
    >
        ...
        @if(($phase ?? null) === 'review')
            ...
            @if(! empty($scores ?? []))
                <div class="mx-auto max-w-xl rounded-xl border border-zinc-200 dark:border-zinc-700 p-6">
        ...
        @else
            ...
            <div class="mx-auto flex max-w-xl items-center justify-between text-zinc-500 dark:text-zinc-400">
                <span>{{ __(':answered / :total answered', ['answered' => $answeredCount ?? 0, 'total' => $totalPlayers ?? 0]) }}</span>
                <span>{{ $currentQuestion['time_limit_seconds'] ?? 30 }}s</span>
            </div>
        @endif
